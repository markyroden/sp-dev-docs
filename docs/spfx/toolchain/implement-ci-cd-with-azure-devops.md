...
Because third party dependencies are not stored in the source control, you need to restore those before starting to build the project. To do so add a `npm` task and set the Command to `custom` and the Command and Arguements to `ci`.
